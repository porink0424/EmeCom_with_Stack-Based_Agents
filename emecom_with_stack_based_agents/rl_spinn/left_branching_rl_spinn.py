...
class LeftBranchingRlSpinn(RlSpinnBase):
    def __init__(
        self,
        D_vec: int,
        D_tracking: int,
    ...
    def forward(self, messages: torch.Tensor):
        ...
        for t_step in range(2, 2 * self.max_length - 1):
            ...
            if t_step <= 1 or t_step % 2 == 1:
                reduce_mask[:, REDUCE] = (
                    torch.ones_like(reduce_mask[:, REDUCE]).to(self.device)
                    * torch.finfo(torch.float32).min
                )
            # mask where shift is not allowed
            shift_mask = torch.zeros_like(transition_logits).to(self.device)
            if t_step >= 2 and t_step % 2 == 0:
                shift_mask[:, SHIFT] = (
                    torch.ones_like(shift_mask[:, SHIFT]).to(self.device)
                    * torch.finfo(torch.float32).min
                )

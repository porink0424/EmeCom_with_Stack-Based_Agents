...
from emecom_with_stack_based_agents.rl_spinn.rl_spinn import RlSpinnBase, RlSpinnOutput
from emecom_with_stack_based_agents.rl_spinn.tracking_lstm import TrackingLstm


class ShiftReduceDecider:
    def __init__(self, max_length: int):
        self.lis: list[int] = []
        shift_count = 0
        while len(self.lis) < 2 * max_length - 1:
            self.lis.append(SHIFT)
            shift_count += 1
            tmp = shift_count
            while tmp % 2 == 0:
                self.lis.append(REDUCE)
                tmp //= 2
        self.lis = self.lis[: 2 * max_length - 1]

    def __call__(self, t_step: int):
        assert t_step < len(self.lis)
        return self.lis[t_step]


class BinaryBranchingRlSpinn(RlSpinnBase):
    def __init__(
        self,
        D_vec: int,
        D_tracking: int,
        ...
        self.shift_reduce_decider = ShiftReduceDecider(max_length)
    ...
    def forward(self, messages: torch.Tensor):
        ...
        for t_step in range(2, 2 * self.max_length - 1):
            ...
            transition_logits = torch.zeros(batch_size, 2).to(self.device)

            decided_transitions = torch.full(
                size=(batch_size,), fill_value=self.shift_reduce_decider(t_step)
            ).to(self.device)
            decided_transitions = torch.where(
                unexecuted_shifts == 0,
                REDUCE,
                decided_transitions,
            )
            ...
            log_prob = torch.zeros_like(transitions).to(self.device)
            entropy = torch.zeros_like(transitions).to(self.device)
